...
for i in "${!depth[@]}";do
    for j in "${!n_ctx[@]}";do
    ## train on the VisA dataset
        base_dir=${depth[i]}_${n_ctx[j]}_${t_n_ctx[0]}_mvtec_anomaly_detection+F1038-F2150-M2520
        save_dir=/home/s1k2/work/checkpoints/AnomalyCLIPTrainSH_${base_dir}/
        CUDA_VISIBLE_DEVICES=${device} python train.py --dataset mvtec --train_data_path /home/s1k2/work/datasets/mvtec_anomaly_detection+F1038-F2150-M2520 \
        --save_path ${save_dir} \
        --features_list 24 --image_size 518  --batch_size 8 --print_freq 1 \
        --epoch 15 --save_freq 1 --depth ${depth[i]} --n_ctx ${n_ctx[j]} --t_n_ctx ${t_n_ctx[0]}
    wait
